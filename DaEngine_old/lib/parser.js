    ...
    if (/\.wxml$/.test(fullPath)) {
      ...
      parseImports(fullPath, true, (err, srcs) => {
        ...
        if (useDefaultCompiler) {
          ...
          // 调用 wcsc.exe 来编译 .wxss文件
          let execWcsc = execFile.bind(null, wcsc, wxssArgs.concat(srcs)) // 此处只是绑定
          if (isLinux) {
            execWcsc = exec.bind(
              null,
              [wcsc]
          ...
          // 设置maxBuffer是为了避免爆栈？（此处是执行，下面的内容是执行完后的回调）
