...
export default window.exparser.registerElement({
    ...
    _gotoSlide: function (next, curr) {
        if (this._slideCount) {
            if ((this._updateDots(next), this.circular && this._slideCount > 1)) {
                var currPos = Math.round(this._viewport), ratio = Math.floor(currPos / this._slideCount), nextPos = ratio * this._slideCount + next;
                curr > 0
    ...
    _normalizeCurrentSlide: function (nextSlide) {
        if (this._slideCount) {
            return ((Math.round(nextSlide) % this._slideCount + this._slideCount) %
                this._slideCount);
        }
        else {
            return 0;
