var _slicedToArray = (function () {
    ...
    return function (obj, length) {
        ...
        if (Symbol.iterator in Object(obj))
            return sliceForIteratorObj(obj, length);
        throw new TypeError('Invalid attempt to destructure non-iterable instance');
    };
})();
export var Video = window.exparser.registerElement({
    is: 'wx-video',
    ...
    actionChanged: function (action, t) {
        if (typeof action === 'object') {
            ...
            if (method === 'play') {
                ...
                var danmuInfo = _slicedToArray(data, 2), txt = danmuInfo[0], color = danmuInfo[1], currentTime = parseInt(this.$.player.currentTime);
    ...
    _computeRate: function (targetPos) {
        var elapsed = this.$.progress.getBoundingClientRect().left, totalLen = this.$.progress.offsetWidth, rate = (targetPos - elapsed) / totalLen;
