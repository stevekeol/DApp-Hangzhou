...
export default window.exparser.registerElement({
    ...
    _ready: function () {
        if (!(this._img && this._img instanceof Image)) {
            ...
            var cb_1 = this._pageReRenderCallback.bind(this);
            document.addEventListener('leavePage', function () {
                document.removeEventListener('pageReRender', cb_1);
            });
            document.addEventListener('pageReRender', cb_1);
        }
    },
    attached: function () {
        this._ready();
    ...
    srcChanged: function (filePath, t) {
        if (filePath) {
            var ua = (this.$.div, window.navigator.userAgent.toLowerCase()), self = this;
            this._ready();
            var opts = {
                success: function (e) {
