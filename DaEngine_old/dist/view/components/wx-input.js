    ...
    window.exparser.registerElement({
        ...
        detached: function () {
            document.removeEventListener('pageReRender', this.__updateInput),
                ...
                exparser.removeListenerFromElement(document, 'hideKeyboard', this.__hideKeyboardId),
                exparser.removeListenerFromElement(document, 'onKeyboardComplete', this.__onKeyboardCompleteId),
                exparser.removeListenerFromElement(document, 'setKeyboardValue', this.__setKeyboardValueId);
        },
        ...
        updateInput: function () {
            var styles = window.getComputedStyle(this.$$), bounds = this.$$.getBoundingClientRect(), pos = (['Left', 'Right'].map(function (type) {
                return (parseFloat(styles['border' + type + 'Width']) +
                    parseFloat(styles['padding' + type]));
            }),
                ['Top', 'Bottom'].map(function (type) {
                    return (parseFloat(styles['border' + type + 'Width']) +
                        parseFloat(styles['padding' + type]));
                })), inputObj = this.$.input, height = bounds.height - pos[0] - pos[1];
        ...
        _checkPlaceholderStyle: function (hide) {
            var phClasss = this._placeholderClass || [], placeholderNode = this.$.placeholder;
            if (hide) {
