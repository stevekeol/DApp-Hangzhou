...
function copyHandle(data) {
    ...
    var coll = organizeValue.copyCollection(data, method), newAttr = coll !== null ? coll : data, attrArr = [data], newAttrArr = [newAttr];
    ...
    for (index = 0, length = keys.length; index < length; ++index) {
        ...
        attrValueIndex === -1
            ? ((newAttrValue = organizeValue.copy(attrValue, method)),
                (curAttrValue = newAttrValue !== null ? newAttrValue : attrValue),
                attrValue !== null &&
                    /^(?:function|object)$/.test(typeof attrValue) &&
                    (attrArr.push(attrValue), newAttrArr.push(curAttrValue)))
            : (tmpObj = newAttrArr[attrValueIndex]);
        newAttr[key] =
            tmpObj ||
                iteratorHandle(attrValue, method, curAttrValue, attrArr, newAttrArr);
    }
    return newAttr;
}
