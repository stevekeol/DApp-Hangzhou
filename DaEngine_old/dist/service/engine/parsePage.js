...
var PageParser = /** @class */ (function () {
    function PageParser() {
        var pageObj = arguments.length <= 0 || void 0 === arguments[0] ? {} : arguments[0], curPage = this, webviewId = arguments[1], routePath = arguments[2];
        var pageBaseAttr = {
        ...
        sysEventKeys.forEach(function (eventName) {
            ...
            curPage[eventName] = function () {
                var eventFun = (pageObj[eventName] || utils.noop).bind(this), res;
    ...
    PageParser.prototype.update = function () {
        utils.warn('将被废弃', 'Page.update is deprecated, setData updates the view implicitly. [It will be removed in 2016.11]');
    };
    PageParser.prototype.forceUpdate = function () {
        utils.warn('将被废弃', 'Page.forceUpdate is deprecated, setData updates the view implicitly. [It will be removed in 2016.11]');
    };
    PageParser.prototype.setData = function (dataObj, callback) {
        try {
            var type = utils.getDataType(dataObj);
            type !== 'Object' &&
                utils.error('类型错误', 'setData accepts an Object rather than some ' + type);
            for (var key in dataObj) {
                var curValue = getObjectByPath(this.data, key), curObj = curValue.obj, curKey = curValue.key;
                curObj && (curObj[curKey] = organize(dataObj[key]));
            }
            var execCallback_1 = function () {
                callback();
                document.removeEventListener('pageReRender', execCallback_1);
            };
            if (callback) {
                document.addEventListener('pageReRender', execCallback_1);
            }
            if (window.reRender) {
