...
var pageIndex = 0;
export var getCurrentPage = function () {
    return currentPage;
};
export var getCurrentPages = function () {
    var pageArr = [];
...
export var pageHolder = function (pageObj) {
    // Page 接口
    if (!__wxRouteBegin) {
    ...
    __wxRouteBegin = !1;
    var pages = __wxConfig__.pages, pagePath = __wxRoute || pages[pageIndex];
    pageIndex++;
    if (utils.getDataType(pageObj) !== 'Object') {
        throw utils.error('Page 注册错误', 'Options is not object: ' +
            JSON.stringify(pageObj) +
            ' in ' +
            __wxRoute +
            '.js');
        new utils.AppServiceEngineKnownError('Options is not object: ' +
            JSON.stringify(pageObj) +
            ' in ' +
            __wxRoute +
            '.js');
...
var isTabBarsPage = function (pageItem) {
    //
    return (tabBars.indexOf(pageItem.route) !== -1 ||
...
var skipPage = function (routePath, pWebViewId, pageParams, pApiKey) {
    ...
    if (pApiKey === 'navigateTo') {
        ...
        isExist ||
            utils.error('Page route 错误(system error)', 'navigateBack with an unexist webviewId ' + pWebViewId);
    }
        ...
        else if ((isTabBarsPage(pageStack[0])
...
var pullDownRefresh = function (pWebviewId) {
    ...
    var pageItem = pageStackObjs[pWebviewId], pageObj = pageItem.page;
...
var invokeShareAppMessage = function (params, pWebviewId) {
    ...
    var shareParams = params, pageItem = pageStackObjs[pWebviewId], pageObj = pageItem.page, eventName = 'onShareAppMessage';
...
export var setWxConfig = function (e) {
    __wxConfig__ = e;
};
export var setWxRoute = function (e) {
    __wxRoute = e;
};
export var setWxRouteBegin = function (e) {
    __wxRouteBegin = e;
};
export var getWebviewIdToPage = function () {
    return pageStackObjs;
};
export var getRouteToPage = function () {
    return pageRegObjs;
};
wx.onAppRoute(Reporter.surroundThirdByTryCatch(function (params) {
    var path = params.path, webviewId = params.webviewId, query = params.query || {}, openType = params.openType;
    skipPage(path, webviewId, query, openType);
}), 'onAppRoute');
wx.onWebviewEvent(Reporter.surroundThirdByTryCatch(function (params) {
    var webviewId = params.webviewId, eventName = params.eventName, data = params.data;
