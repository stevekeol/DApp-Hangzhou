"use strict";
(self["webpackChunkDaEngine"] = self["webpackChunkDaEngine"] || []).push([[425],{
...
//   longtap: 'onLongPress'
// }
var LONG_PRESS_TIME_THRESHOLD = 300;
var LONG_PRESS_DISTANCE_THRESHOLD = 5;
// const format = function (obj, method, arr, key) {
...
var getCanvasTouches = function getCanvasTouches(args) {
  var self = this;
...
var calcDistance = function calcDistance(end, start) {
...
exports["default"] = window.exparser.registerElement({
  ...
  attached: function attached() {
    ...
    if (window.__canvasNumbers__.hasOwnProperty(canvasId)) {
      ...
      window.__canvasNumbers__[canvasId] = window.__canvasNumber__ + __webviewId__;
  ...
  addTouchEventForWebview: function addTouchEventForWebview() {
    ...
    touchEventNames.forEach(function (eventName) {
      self.$$.addEventListener(eventName, function (event) {
        ...
        var changedTouches = getCanvasTouches.call(self, event.changedTouches);
        if (self.bindlongtap) {
          ...
          if (eventName === 'touchstart') {
            changedTouches.forEach(function (curEvent) {
              ...
              self._touchInfo[curEvent.identifier].timeStamp = event.timeStamp;
              ...
              self._touchInfo[curEvent.identifier].handler = handler;
            });
          } else {
            eventName === 'touchend' || eventName === 'touchcancel' ? changedTouches.forEach(function (n) {
              self._touchInfo.hasOwnProperty(n.identifier) || console.error('in ' + eventName + ', can not found ' + n.identifier + ' in ' + (0, _stringify2.default)(self._touchInfo));
              self._touchInfo[n.identifier].longPress && --self._disableScroll;
              clearTimeout(self._touchInfo[n.identifier].handler);
              delete self._touchInfo[n.identifier];
            }) : changedTouches.forEach(function (n) {
              self._touchInfo.hasOwnProperty(n.identifier) || console.error('in ' + eventName + ', can not found ' + n.identifier + ' in ' + (0, _stringify2.default)(self._touchInfo));
              calcDistance(self._touchInfo[n.identifier], n) > LONG_PRESS_DISTANCE_THRESHOLD && !self._touchInfo[n.identifier].longPress && clearTimeout(self._touchInfo[n.identifier].handler);
  ...
  actionsChanged: function actionsChanged(actions, flag) {
    if (actions) {
