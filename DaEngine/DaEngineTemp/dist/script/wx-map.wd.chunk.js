"use strict";
(self["webpackChunkDaEngine"] = self["webpackChunkDaEngine"] || []).push([[336],{
...
window.exparser.registerElement({
  ...
  _transformPath: function _transformPath(path, t) {
    return path.map(function (item) {
      var tempObj = {};
      return item.iconPath ? ((0, _keys2.default)(item).forEach(function (itemName) {
  ...
  _transformMarkers: function _transformMarkers(markers) {
    ...
    return (markers || []).map(function (marker) {
      var tempObj = {};
      return marker ? ((0, _keys2.default)(marker).forEach(function (t) {
        tempObj[t] = marker[t];
  ...
  _transformControls: function _transformControls(ctrls) {
    ...
    return ctrls.map(function (ctrl) {
      var tempObj = {};
      (0, _keys2.default)(ctrl).forEach(function (t) {
        tempObj[t] = ctrl[t];
      });
  ...
  latitudeChanged: function latitudeChanged(centerLatitude, ori) {
    if (centerLatitude) {
      this._isReady ? this._map.setCenter(new qq.maps.LatLng(centerLatitude, this.longitude)) : this._delay('latitudeChanged', centerLatitude, ori);
    }
  },
  longitudeChanged: function longitudeChanged(centerLongitude, ori) {
    if (centerLongitude) {
      this._isReady ? this._map.setCenter(new qq.maps.LatLng(this.latitude, centerLongitude)) : this._delay('longitudeChanged', centerLongitude, ori);
    }
  },
  scaleChanged: function scaleChanged() {
    var scale = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 16;
    var ori = arguments[1];

    if (scale) {
      this._isReady ? this._map.zoomTo(scale) : this._delay('scaleChanged', scale, ori);
    }
  },
  coversChanged: function coversChanged() {
    var covers = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    var ori = arguments[1];
  ...
  markersChanged: function markersChanged() {
    var markerArg = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    var ori = arguments[1];
    ...
    if (this._isReady) {
      ...
      this._markers = markerArg.map(function (markerItem) {
        ...
        markerItem.iconPath && (Number(markerItem.width) && Number(markerItem.height) ? markerIns.setIcon(new qq.maps.MarkerImage(markerItem.iconPath, new qq.maps.Size(markerItem.width, markerItem.height), new qq.maps.Point(0, 0), new qq.maps.Point(markerItem.width / 2, markerItem.height), new qq.maps.Size(markerItem.width, markerItem.height))) : markerIns.setIcon(new qq.maps.MarkerImage(markerItem.iconPath)));(markerItem.title || markerItem.name) && markerIns.setTitle(markerItem.title || markerItem.name);
        self.bindmarkertap && typeof markerItem.id !== 'undefined' && qq.maps.event.addListener(markerIns, 'click', function (n) {
          var event = n.event;
  ...
  circlesChanged: function circlesChanged() {
    var circles = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    var ori = arguments[1];
  ...
  pointsChanged: function pointsChanged() {
    ...
    var self = this;
    if (!this._isReady) this._delay('pointsChanged', points, ori);(function () {
      if (points.length <= 0) {
        return;
      }
  ...
  controlsChanged: function controlsChanged() {
    var nCtrl = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    var n = arguments[1];
  ...
  showLocationChanged: function showLocationChanged(location, uselessArg) {
    var self = this;
    this._isReady ? (this._posOverlay && (this._posOverlay.setMap(null), this._posOverlay = null), location && WeixinJSBridge.invoke('private_geolocation', {}, function (res) {
      try {
        res = JSON.parse(res);
      } catch (e) {
        res = {};
  ...
  attached: function attached() {
    if (this.latitude > 90 || this.latitude < -90) {
      ...
      window.__map_jssdk_ready ? this._insertIframeMap() : window.__map_jssdk_callback.push(this._insertIframeMap.bind(this));
      return void 0;
    }
  },
