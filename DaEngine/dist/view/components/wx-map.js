...
window.exparser.registerElement({
    ...
    _transformColor: function (hexColor) {
        ...
        var r = Number('0x' + hexColor.substr(0, 2)), g = Number('0x' + hexColor.substr(2, 2)), b = Number('0x' + hexColor.substr(4, 2)), a = hexColor.substr(6, 2) ? Number('0x' + hexColor.substr(6, 2)) / 255 : 1;
    ...
    _initFeatures: function () {
        this._mapId &&
            (((this.markers && this.markers.length > 0) ||
                ...
                WeixinJSBridge.invoke('addMapMarkers', {
                    mapId: this._mapId,
    ...
    _insertIframeMap: function () {
        var self = this, map = (this._map = new qq.maps.Map(this.$.map, {
            zoom: this.scale,
            center: new qq.maps.LatLng(this.latitude, this.longitude),
            mapTypeId: qq.maps.MapTypeId.ROADMAP,
            zoomControl: !1,
            mapTypeControl: !1
        })), isDragging = !1, stopedDragging = !1;
        ...
        CustomOverlay.prototype.draw = function () {
            var projection = this.getProjection(), pixInfo = projection.fromLatLngToDivPixel(this.position), style = this.div.style;
    ...
    latitudeChanged: function (centerLatitude, ori) {
        if (centerLatitude) {
            this._isReady
                ? this._map.setCenter(new qq.maps.LatLng(centerLatitude, this.longitude))
    ...
    longitudeChanged: function (centerLongitude, ori) {
        if (centerLongitude) {
            this._isReady
                ? this._map.setCenter(new qq.maps.LatLng(this.latitude, centerLongitude))
                : this._delay('longitudeChanged', centerLongitude, ori);
        }
    },
    scaleChanged: function (scale, ori) {
        if (scale === void 0) { scale = 16; }
        if (scale) {
            this._isReady
            ...
            this._markers = markerArg.map(function (markerItem) {
                ...
                markerItem.iconPath &&
                    (Number(markerItem.width) && Number(markerItem.height)
                        ? markerIns.setIcon(new qq.maps.MarkerImage(markerItem.iconPath, new qq.maps.Size(markerItem.width, markerItem.height), new qq.maps.Point(0, 0), new qq.maps.Point(markerItem.width / 2, markerItem.height), new qq.maps.Size(markerItem.width, markerItem.height)))
    ...
    pointsChanged: function (points, ori) {
        ...
        var self = this;
        if (!this._isReady)
            this._delay('pointsChanged', points, ori);
        (function () {
            if (points.length <= 0) {
                return;
            }
    ...
    controlsChanged: function (nCtrl, n) {
        ...
        if (this._isReady) {
            ...
            this._delay('controlsChanged', nCtrl, n);
    },
